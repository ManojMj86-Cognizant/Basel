"""Offline additive-rule satisfaction check: evaluate every scoped additive rule against the
built instance facts (Arelle aborts formula eval on the missing xfm:log function, so we measure
the additive class directly via the rc-code bridge)."""
import sys, json
...
def fkey(concept, dims):  # (metric-local, frozenset (dimlocal,memlocal))
    return (dim_drs.local(concept), frozenset((dim_drs.local(k), dim_drs.local(v)) for k,v in dims.items()))

# build fact value map from instance
...
for r in rules:
    if not r["tables"] or not (set(r["tables"])<=tset) or r.get("deactivated"): continue
    for a in workbook_rules.expand_scoped_asts(r):
        ...
        if missing: incomplete+=1; continue
        if abs(lhs-rhs)<0.5: balanced+=1
        else: unbal+=1
print(f"scoped additive equations (i=) over PRA001: {total}")
print(f"  balanced (satisfied):   {balanced}")
print(f"  UNBALANCED (fail):      {unbal}")
print(f"  incomplete (a cell has no fact in instance): {incomplete}")
