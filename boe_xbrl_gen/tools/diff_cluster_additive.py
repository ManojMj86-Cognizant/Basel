"""Diff the cluster ADDITIVE failing-rule SETS between two instances (load rules once, eval both).
Confirms whether an emit ADDED any new failing rule (activation/regression) vs only REMOVED failures.
Usage: python tools/diff_cluster_additive.py <base.xbrl> <new.xbrl>"""
import sys, json
...
F_BASE, F_NEW = sys.argv[1], sys.argv[2]
...
def failing_codes(facts):
    ...
    for r in R:
        ...
        for a in workbook_rules.expand_scoped_asts(r):
            ...
            if abs(s) >= 0.5:
                bad.add(r["code"]); break
    return bad


b = failing_codes(load_facts(F_BASE))
n = failing_codes(load_facts(F_NEW))
print(f"base {F_BASE.split(chr(92))[-1]}: {len(b)} failing additive rules")
print(f"new  {F_NEW.split(chr(92))[-1]}: {len(n)} failing additive rules")
print(f"\nFIXED by new (in base, not new): {len(b - n)}")
print(f"NEW failures in new (regressions/activations): {len(n - b)}")
if n - b:
    print("  ->", sorted(n - b))
